...
control_user_led1
     ...
     // save the input argument and return address
    PUSH {R0-R1,LR} 
    
    //Assign the out data register to R3, base plus offset
    LDR.N R3, =GPIOA_BASE
    ADD R3, R3, #GPIOA_ODR
    
    //initialize register R2, and assign  --> (1<<5)or (0<<5)
    MOV R2,#0
    MOV R2, R0,LSL#5
    
    //assign the back to R0 --> R0 = 0x20 or decimal #32
    ORRS.W R0,R2,R2
    
    //Write to the memory of address R3 to turn LED ON and OFF
    STR R0, [R3] 
   
   // assign the delay duration to R0 to pass it delay function
    MOV R0, R1
